...
void Visitor::visit(const CompUnits &node)
{
    for (auto &elm : node)
    {
        std::visit(overloaded{
                       [this](const Decl &node)
                       { visit_decl(node); },
                       [this](const FuncDef &node)
                       { visit_func_def(node); },
                   },
                   *elm);
    }
...
bool Visitor::_has_control_stmt(const Stmt &node)
{
    return std::visit(
        overloaded{
            [this](const AssignStmt &node)
            { return false; },
            [this](const ExpStmt &node)
            { return false; },
            ...
            { return false; },
            [this](const ControlStmt &node)
            { return true; },
            [this](const ReturnStmt &node)
            { return false; },
        },
        node);
}
...
ExpReturn Visitor::visit_exp(const Exp &node)
{
    return std::visit(
        overloaded{
            [this](const BinaryExp &node)
            { return visit_binary_exp(node); },
            [this](const LValExp &node)
            { return visit_lval_exp(node); },
            [this](const CallExp &node)
            { return visit_call_exp(node); },
            [this](const UnaryExp &node)
            { return visit_unary_exp(node); },
            [this](const CompareExp &node)
            { return visit_compare_exp(node); },
            [this](const Number &node)
...
ExpReturn Visitor::visit_binary_exp(const BinaryExp &node)
{
    ...
    {
    case BinaryExp::ADD:
        if (auto const_left = std::dynamic_pointer_cast<ir::ConstBits>(left_val))
        {
            if (std::get_if<int>(&const_left->value) && std::get<int>(const_left->value) == 0)
            {
                return ExpReturn(type, right_val);
            }
        }
        if (auto const_right = std::dynamic_pointer_cast<ir::ConstBits>(right_val))
        {
            if (std::get_if<int>(&const_right->value) && std::get<int>(const_right->value) == 0)
            {
                return ExpReturn(type, left_val);
            }
        }
    ...
    case BinaryExp::MULT:
        if (auto const_left = std::dynamic_pointer_cast<ir::ConstBits>(left_val))
        {
            if (std::get_if<int>(&const_left->value) && std::get<int>(const_left->value) == 1)
            {
                return ExpReturn(type, right_val);
            }
            if (std::get_if<int>(&const_left->value) && std::get<int>(const_left->value) == 0)
        ...
        if (auto const_right = std::dynamic_pointer_cast<ir::ConstBits>(right_val))
        {
            if (std::get_if<int>(&const_right->value) && std::get<int>(const_right->value) == 1)
            {
                return ExpReturn(type, left_val);
            }
            if (std::get_if<int>(&const_right->value) && std::get<int>(const_right->value) == 0)
...
ExpReturn Visitor::visit_call_exp(const CallExp &node)
{
    ...
    if (node.ident == "starttime" || node.ident == "stoptime")
    { // very ugly
        return ExpReturn(
            symbol->type,
...
CondReturn Visitor::visit_cond(const Cond &node)
{
    return std::visit(
        overloaded{
            ...
            [this](const LogicalExp &node)
            { return visit_logical_exp(node); },
        },
        node);
}
